...
def run_splitter(java_executable: Path, config: dict, input_file_path: Path, output_directory_path: Path) -> Dict[str, Path]:
...
def process_material(args):
    ...
    if material:
        ...
        material_id = get_material_from_file(config['voxelizer'], str(osm2world_output_file_path) + '.mtl', material_name)
...
def main(config, region_x, region_z, region_directory_path: Path):
    ...
    with tempfile.TemporaryDirectory() as temporary_directory_path_plain:
        ...
        region_max_bound = np.array([region_size_x / 2 - 1, config['voxelizer']['max_y']+config['map']['min_height']-config['voxelizer']['min_y'], region_size_z / 2 - 1])
        
        region_offset = np.array([region_center_x, config['voxelizer']['min_y']-config['map']['min_height'], region_center_z])
        ...
        with ProcessPoolExecutor() as executor:
            materials_voxels = executor.map(process_material, ((material_name, material_mesh_path, material_dictionary, osm2world_output_file_path, config,
                    region_min_bound, region_max_bound, region_offset, region_size_x, region_size_z, region_center_x, region_center_z,
        ...
        if terrain_material_objects:
            ...
            for terrain_material_object in terrain_material_objects:
                terrain_voxels_list.extend(voxelize_mesh(terrain_material_object[1], region_min_bound, region_max_bound, region_offset))
            ...
            if voxels:
                for (x, y, z), block in voxels:
                    ...
                    if (height_matrix and terrain_matrix and 
                       ...
                       (height_matrix[z][x] is None or y < height_matrix[z][x])) and block.id in config['voxelizer']['terrain_interpolator_markers']:
                        height_matrix[z][x] = y
                        
                    
        if terrain_material_object is not None:
            terrain_voxels_list = make_terrain(height_matrix, config['voxelizer']['min_y'],
                                    int(region_center_x - region_size_x / 2), int(region_center_z - region_size_z / 2),
                                    terrain_material_block, anvil.Block('minecraft', 'bedrock'), terrain_blocks)
